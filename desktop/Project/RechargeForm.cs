...
namespace SQ_Email_Tools
{
    ...
    public class RechargeForm : Form
    {
        ...
        private const long CYCLE = 10_000L;
        ...
        private Panel     _pnlSingleContent;   // 右側：新增儲值
        ...
        private void RebuildPlayerInfo()
        {
            ...
            _pnlPlayerInfo.Controls.Add(new Label
            {
                Text = $"累積儲值進度（第 {cycNum + 1} 輪）  ·  每累積 NT$10,000 完成一輪，可領大獎",
                ForeColor = Theme.TextMuted, Font = Theme.FontSmall,
        ...
        // 鈹€鈹€ 鏂板鍎插€煎収瀹癸紙鍘?BuildRightPanel 涓婚珨绉昏嚦姝わ級鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
        ...
        private async Task SearchPlayerAsync()
        {
            ...
            try
            {
                ...
                if (pickedList.Count > 1)
                    ShowMsg($"✓ 已選取 {pickedList.Count} 位，僅顯示第一位：{_detail.OnlineName}（{_detail.Account}）", true);
                else
        ...
        private void UpdatePreview()
        {
            ...
            string cycleNote = gained > 0
                ? $"循環：{curPt:N0} → {aftIn:N0}/10,000　（完成 {gained} 輪！）"
                : $"循環：{curPt:N0} → {afterPt:N0}/10,000";
