...
namespace SQ_Email_Tools
{
    public class SendForm : Form
    {
        ...
        private void BuildRightPanel(Panel p)
        {
            ...
            // ── STEP 2 購物車標題列（TableLayoutPanel：左 Label + 右兩顆按鈕）──
            ...
            var btnClearCart = Theme.MakeButton("🗑 清空", Theme.AccentRed, Color.White, 64, 26);
            btnClearCart.Font   = Theme.FontSmall;
            btnClearCart.Margin = new Padding(2, 1, 2, 1);
            ...
            cartHdrPanel.Controls.Add(btnClearCart,   2, 0);
            scroll.Controls.Add(cartHdrPanel);
            y += 36;

            // ── 醒目的「上傳道具清單」大按鈕（標題下方獨立一條，絕對看得到）──
        ...
        // ═══════════════════════════════════════════════════════════
        // 一鍵上傳道具清單到購物車
        // 支援 CSV / TXT / Excel；欄位偵測：編號(必)、數量(預設1)、Type(預設0)
        // ═══════════════════════════════════════════════════════════
        private void UploadItemsToCart()
        {
            ...
            // 多分頁 Excel → 跳分頁選擇器（單一分頁或非 Excel 不會跳）
            ...
            {
                // 依 Type 決定查 item 還是 pet（type=2 為寵物）；若查不到再 fallback FindItemById
                ...
                var existing = _cart.Find(c => c.Item.Id == r.Id && c.Type == r.Type);
                if (existing != null)
                {
                    existing.Qty = Math.Max(1, existing.Qty + r.Qty);
                    merged++;
                }
                else
                {
                    _cart.Add(new CartEntry { Item = info, Qty = Math.Max(1, r.Qty), Type = r.Type });
                    added++;
                }
            }
            RefreshCartDgv();
            ...
            ShowStatus($"✓ 上傳完成：新增 {added} 種、累加 {merged} 種、未匹配 {missing}", null);
        }

        // 友善的上傳結果報告（含欄位偵測 + 跳過明細）
