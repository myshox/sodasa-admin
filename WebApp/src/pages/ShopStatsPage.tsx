...
export default function ShopStatsPage() {
  ...
  return (
    <div style={{ padding: 28 }}>
      <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 20 }}>🏪 {S.navShop}</h1>
      ...
      {loading ? <p style={{ color: 'var(--text-muted)' }}>載入中…</p> : data && (
        <>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', marginBottom: 24 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '60px 80px 1fr 100px 100px 120px 140px', padding: '10px 16px', background: 'var(--bg-sidebar)', fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>
            ...
            {data.items.length === 0 ? <p style={{ padding: 24, color: 'var(--text-muted)', textAlign: 'center' }}>尚無購買記錄</p> : data.items.map((row: any) => (
              <div key={row.rank} style={{ display: 'grid', gridTemplateColumns: '60px 80px 1fr 100px 100px 120px 140px', padding: '10px 16px', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 1fr 100px 120px', padding: '10px 16px', background: 'var(--bg-sidebar)', fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>
            ...
            {data.spenders.length === 0 ? <p style={{ padding: 24, color: 'var(--text-muted)', textAlign: 'center' }}>尚無記錄</p> : data.spenders.map((row: any) => (
              <div key={row.rank} style={{ display: 'grid', gridTemplateColumns: '60px 1fr 1fr 100px 120px', padding: '10px 16px', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
