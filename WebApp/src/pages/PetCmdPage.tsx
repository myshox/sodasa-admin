...
export default function PetCmdPage() {
  ...
  const Nud = ({ label, value, onChange, min = 0, max = 99999 }: {
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 8, marginBottom: 10, flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
      <span style={{ width: isMobile ? 90 : 110, color: 'var(--text-muted)', fontSize: 13, textAlign: 'right', flexShrink: 0 }}>
        {label}
      </span>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button onClick={() => onChange(Math.max(min, value - 1))}
          style={{ background: 'var(--bg-input)', color: 'var(--text-secondary)', border: '1px solid var(--border)', minWidth: isMobile ? 44 : undefined, minHeight: isMobile ? 44 : undefined, padding: isMobile ? '10px 14px' : '4px 12px' }}>
          {S.minus}
        </button>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button onClick={() => onChange(Math.max(0, parseFloat((value - 0.001).toFixed(3))))}
          style={{ background: 'var(--bg-input)', color, border: '1px solid var(--border)', minWidth: isMobile ? 44 : undefined, minHeight: isMobile ? 44 : undefined, padding: isMobile ? '10px 14px' : '4px 12px' }}>
          {S.minus}
        </button>
        <input type="number" value={value} step={0.001} min={0} max={99.999}
          ...
          style={{ width: isMobile ? 72 : 100, textAlign: 'center', color }} />
  ...
  const Card = ({ title, accent, children }: { title: string; accent?: string; children: React.ReactNode }) => (
    <div style={{
      ...
      borderRadius: 10, padding: isMobile ? 16 : 20, marginBottom: 16
    }}>
  ...
  const CmdBar = ({ cmd, ckey }: { cmd: string; ckey: string }) => {
    ...
    return (
      <div style={{
        ...
        gap: 8,
        alignItems: isMobile ? 'stretch' : (isMultiLine ? 'flex-start' : 'center'),
        marginTop: 12,
      }}>
        {isMultiLine
